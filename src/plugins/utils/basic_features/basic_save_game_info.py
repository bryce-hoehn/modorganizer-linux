...
from PySide6.QtCore import QDateTime, QFileInfo, QLocale, Qt
...
def format_date(date_time: QDateTime | datetime | str, format_str: str | None = None):
    ...
    if isinstance(date_time, str):
        if format_str is None:
            # Use ISO date format as default when no format is specified
            date_time = QDateTime.fromString(date_time, Qt.ISODate)
        else:
            date_time = QDateTime.fromString(date_time, format_str)
    elif isinstance(date_time, datetime):
        # Convert Python datetime to QDateTime
        date_time = QDateTime.fromSecsSinceEpoch(int(date_time.timestamp()))
...
class BasicGameSaveGameInfoWidget(mobase.ISaveGameInfoWidget):
    ...
    def setSave(self, save: mobase.ISaveGame):
        # Handle all possible return types from getFilepath()
        filepath = save.getFilepath()
        if isinstance(filepath, QFileInfo):
            save_path = Path(filepath.filePath())
        else:
